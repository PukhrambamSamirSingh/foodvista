...
const Profile = () => {
    ...
    return (
        <div className="w-full h-full flex flex-col md:flex-row justify-center gap-8">
            <div className="w-full md:w-1/3 flex flex-col gap-4">
                <div className="flex flex-col gap-2 font-bold text-3xl lg:text-4xl">
                    <h1>Welcome</h1>
                    <h1 className="text-orange-500 italic">{user && user.name}</h1>
                </div>
                <ProfileImg />
            </div>
            {user && (
                    ...
                    <div className="w-full flex flex-col gap-2">
                        ...
                        {!user.desc ? (
                            <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                                <textarea className="w-full bg-transparent outline-none p-2 border rounded-md" name="desc" id="desc" placeholder="Write about yourself" cols="30" rows="6" onChange={(e) => setDesc(e.target.value)} />
                        ...
                        <div className="pt-2 w-full">
                            <button onClick={logout} className="w-full text-blue-500 p-2 border rounded-md hover:bg-orange-500">Logout</button>
                        </div>
                    </div>
                </div>
            )}
